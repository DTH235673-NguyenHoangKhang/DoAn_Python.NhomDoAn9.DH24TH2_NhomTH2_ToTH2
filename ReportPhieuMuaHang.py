...
PAGE_SIZE = A4
def export_to_pdf(sophieu, ngaylap,tennv, tenkh, diachi, sdt, tongtien, items):
    if not os.path.exists(TEMP_DIR):
        os.makedirs(TEMP_DIR)
    file_name = f"PhieuMuaHang_InTruoc_{sophieu}.pdf"
    ...
    try:
        ...
        c.drawCentredString(width/2, y_start, "PHIẾU MUA HÀNG")
        ...
        y_start -= line_height
        c.drawString(margin, y_start, f"Khách hàng: {tenkh}")
        y_start -= line_height
        ...
        y_start -= line_height
        # Header bảng
        headers = ["Tên Thuốc", "Số lượng", "Giá bán","Đơn vị","Giảm giá", "Thành tiền"]
        col_widths = [200,50, 50, 50,50, 100]
        x_pos = margin
        c.setFont(FONT_NAME, 10)
