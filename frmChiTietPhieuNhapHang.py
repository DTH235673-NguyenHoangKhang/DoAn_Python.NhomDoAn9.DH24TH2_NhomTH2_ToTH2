...
def getdata_tenthuoc():
    ...
    gettenthuoc.close()
    return tenthuoc
class CTPNH(tk.Toplevel):
    def __init__(self,master,maphieu):
        tk.Toplevel.__init__(self,master)
        center_window(self,600,450)
        self.title("Chi tiết phiếu nhập hàng")
        self.selected_loaithuoc=tk.StringVar()
        self.selected_tenthuoc=tk.StringVar()
        lbl_title = tk.Label(self, text="CHI TIẾT PHIẾU NHẬP HÀNG", font=("Arial", 18, "bold"))
        lbl_title.pack(pady=10)
        frame_info=tk.Frame(self)
        frame_info.pack(pady=10)
        tk.Label(frame_info,text="Mã phiếu nhập hàng").grid(row=0,column=0,padx=5,pady=5,sticky="w")
        ...
        self.cbb_tenthuoc.bind("<<ComboboxSelected>>",self.update_gianhap)

        tk.Label(frame_info,text="Giá nhập").grid(row=1,column=2,padx=5,pady=5,sticky="w")
        self.entry_gianhap=tk.Entry(frame_info,width=25)
        ...
        columns = ("STT","Mã phiếu", "Mã thuốc", "Tên thuốc", "Số lượng","Đơn vị", "Giá nhập", "Thành tiền")
        ...
        tree.column("Giá nhập", width=100, anchor="center")
    ...
    def select_record(self,event):
      ...
      self.entry_gianhap.insert(0,values[6])
    ...
    def load_data(self):
        ...
        conn = connect_db()
        cur = conn.cursor()
        sql_query = ("select c.stt,c.sophieunhaphang,t.MaThuoc,t.tenthuoc,c.SoLuong,d.tendonvi,t.Gianhap,c.ThanhTien "
                    +"from thuoc t, chitietphieunhaphang c,donvitinh d "
                    +"where t.mathuoc=c.mathuoc and t.madvt=d.madvt and sophieunhaphang=? order by c.stt ASC")
    ...
    def Them(self):
        ...
        try:
            cur.execute("INSERT INTO chitietphieunhaphang VALUES (?,?,?,?,?)",
            (maphieu, mathuoc, soluong,stt,thanhtien))
            conn.commit()
    ...
    def Xoa(self):
        ...
        conn = connect_db()
        cur = conn.cursor()
        cur.execute("DELETE FROM chitietphieunhaphang WHERE mathuoc=? and sophieunhaphang=?", (mathuoc,maphieu,))
        conn.commit() 
        sql_update_stt = "UPDATE chitietphieunhaphang SET stt = stt - 1 WHERE sophieunhaphang = ? AND stt > ?"
        cur.execute(sql_update_stt, (maphieu, stt))
        conn.commit()
        thanhtien=tree.item(selected)["values"][7]
        tongtien=int(self.entry_tongtien.get())-thanhtien
        self.entry_tongtien.delete(0,tk.END)
        self.entry_tongtien.insert(0,tongtien)
    ...
    def Sua(self):
        ...
        gianhap = self.entry_gianhap.get()
        donvi = self.entry_donvi.get()
        if loaithuoc == ""  or tenthuoc== "" or gianhap=="" or soluong=="" or donvi=="":
                messagebox.showwarning("Thiếu dữ liệu", "Vui lòng nhập đủ thông tin")
                return
        thanhtien = int(gianhap)*int(soluong)
        tree=self.tree
        ...
        if not values:
            ...
            return
        values = tree.item(selected)["values"]
        tree.item(selected, values=(stt,maphieu, mathuoc, tenthuoc,soluong, donvi, gianhap, thanhtien))
        tongtien=0
        for i in tree.get_children():
    ...
    def Luu(self):
        ...
        cur.execute("update phieunhaphang set tongtien=? where sophieunhaphang=?",(tongtien,maphieu,))
        conn.commit()
        stt=len(tree.get_children())+1
        conn.close()
    def update_gianhap(self, event):
        ...
        if tenthuoc_chon:
            ...
            cur = conn.cursor()
            try:
                sql_gianhap = "SELECT gianhap FROM thuoc WHERE Tenthuoc = ?"
                cur.execute(sql_gianhap, (tenthuoc_chon,))
                gianhap_result = cur.fetchone()
                if gianhap_result:
                    gianhap = gianhap_result[0]
        ...
        self.entry_gianhap.delete(0, tk.END)
        self.entry_donvi.delete(0,tk.END)
        if gianhap:self.entry_gianhap.insert(0, gianhap)
        else:pass 
        if donvi:self.entry_donvi.insert(0,donvi)
        else:pass
    def update_tenthuoc(self,event):
        ...
        cursor.close()
        gettenthuoc.close()
        self.cbb_tenthuoc.set('') # Hoặc self.selected_tenthuoc.set('')
        self.cbb_tenthuoc['values']=tenthuoc
        self.entry_gianhap.delete(0, tk.END)
    def XuatPhieuNhap(self):
        conn = connect_db()
        cur = conn.cursor()
        maphieu=int(self.entry_maphieu.get())
        self.Luu()
        messagebox.showinfo("Thông báo","Xuất phiếu nhập thành công!")
        self.check_xpn=1
        conn.commit()
        self.destroy()
    def Thoat(self):
        ...
        cur = conn.cursor()
        if self.check_xpn==0:
            r=messagebox.askyesno("Cảnh báo","Bạn cần nhấn xuất phiếu nhập trước khi thoát! Nếu thoát ngay lúc này đơn hàng sẽ bị hủy! Bạn có thật sự muốn thoát?")
            if r==False:
                return
        maphieu=int(self.entry_maphieu.get())
        sql_delete="delete from chitietphieunhaphang where sophieunhaphang=?"
